...
async function uploadToDrive(pdfBase64, fileName) {
    const folderID = process.env.GOOGLE_DRIVE_PROBATION_FOLDER_ID;
    if (!folderID) throw new Error('GOOGLE_DRIVE_PROBATION_FOLDER_ID missing in .env');
    ...
    try {
        ...
        for (const candidate of candidates) {
            try {
                ...
                const fileName = `${safeName}_ProbationConfirmation.pdf`;

                let existingRecord = await ProbationConfirmation.findOne({ employeeName: candidate.employeeName });

                // Drive file deleted → remove DB record
                if (existingRecord) {
                    const fileExists = await checkFileExistsInDrive(existingRecord.driveFileId);
                    if (!fileExists) {
                        await ProbationConfirmation.findByIdAndDelete(existingRecord._id);
                        existingRecord = null;
                    }
                }
                ...
                if (existingRecord) {
                    const isDuplicate =
                        ...
                        existingRecord.reportingManager === candidate.reportingManager &&
                        existingRecord.annualHike === candidate.annualHike &&
                        existingRecord.annualPackage === candidate.annualPackage &&
                        existingRecord.plannedLeaves === candidate.plannedLeaves;
                    ...
                    Object.assign(existingRecord, {
                        ...
                        annualHike: candidate.annualHike,
                        plannedLeaves: candidate.plannedLeaves,
                    ...
                    const newRecord = new ProbationConfirmation({
